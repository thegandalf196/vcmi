...
SecondarySkill parentSkill(const std::string & key)
{
	...
	require(id.has_value() && *id == SecondarySkill::ARTILLERY, "unsupported parent skill " + key);
...
void validateMasteryRules(const JsonNode & rules)
{
	...
	require(integer(rules["rulesetVersion"], 1, 1), "rulesetVersion");
	require(rules["skills"].isStruct() && rules["skills"].Struct().size() == 1, "version1 Artillery family required");
...
void validateMasteryOffer(const MasteryOffer & offer)
{
	if(offer.hero.getNum() < 0 || !offer.player.isValidPlayer()
		|| offer.skill != SecondarySkill::ARTILLERY || offer.level < 2 || offer.sequence == 0)
