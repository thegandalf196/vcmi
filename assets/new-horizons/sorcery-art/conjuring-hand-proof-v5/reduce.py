...
d.text((8,8),'V5 CONJURING HAND / NOT APPROVED',fill='white')
...
# Comparison reads only Artist's own prior proof; never modifies it.
previous=ROOT.parent/'illustrated-proof-v4-keyfix'/'output'
comparison=Image.new('RGB',(320,208),'#706351');cd=ImageDraw.Draw(comparison)
for row,(folder,label) in enumerate([(previous,'V4: book/key NOT approved Sorcery'),(OUT,'V5: hand shaping magic TEST')]):
 y=8+row*100;cd.text((8,y),label,fill='white')
 for size,x in [(44,12),(32,89)]:
  a=Image.open(folder/f'native-{size}.png');comparison.paste(a,(x,y+30),a);cd.text((x+size+4,y+40),str(size),fill='white')
comparison.save(OUT/'native-comparison.png')
