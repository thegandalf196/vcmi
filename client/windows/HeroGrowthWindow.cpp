...
HeroGrowthWindow::HeroGrowthWindow(const CGHeroInstance & hero, const newHorizonsHeroes::PrimaryGrowthView & growth)
	: CWindowObject(BORDERED)
{
	...
	elements.push_back(std::make_shared<CLabel>(350, 91, FONT_SMALL, ETextAlignment::CENTER, Colors::WHITE, "Saved hero profile; values at opening"));
	...
	for(size_t i = 0; i < primaryImages.size(); ++i)
	{
		...
		elements.push_back(std::make_shared<CLabel>(x + 77, 164, FONT_MEDIUM, ETextAlignment::CENTER, Colors::YELLOW, "Total: " + std::to_string(growth.modified[i]), 135));
	...
	addSmallLabel(22, 343, 656, "Additional skill growth - independent chances");
	std::string chances;
	...
	elements.push_back(std::make_shared<CTextBox>("Base includes level/quest gains; Total includes items/effects.\nClass start may differ from authored stats; growth proposals are before the cap.\nPrimary cap: " + std::to_string(growth.maximumPrimary)
		+ "; Power scaling divisor: " + std::to_string(growth.powerDivisor) + ".",
		Rect(22, 484, 560, 58), 0, FONT_SMALL, ETextAlignment::TOPLEFT, Colors::WHITE));
