...
The audit used canonical registries and current source bindings rather than counting files. It covers 69 active combat-school spells across the six New Horizons schools plus the five Neutral Adventure Spells, all 31 registered secondary skills, all 310 perk definitions (59 active and 251 planned), all eight Order bindings, and the scoped hero, spellbook, convenience, creature-info, and ranked-recruitment UI surfaces below. The inactive Clone roster entry is excluded. The unregistered Magic Missile and Spell Lock definitions are noted separately as non-live.
...
| Active perk icon gaps (15) | Provisional | Not done | Fourteen active perks resolve to the neutral fallback; Stormcaller uses the Havoc skill glyph. Bind role-specific art and state frames. |
| Planned perk definitions (251) | Not done | Not done | All planned entries are inventoried individually. They are not active features and have no role-specific runtime binding; neutral fallback is not artwork. Reassess as each effect moves to active. |
...
| Blue-buffer spell-points UI | Not done | Not done | The requested blue-buffer treatment has no current UI or art binding. [HeroInfoWindow.cpp](../client/battle/HeroInfoWindow.cpp) still displays spell points as plain current/max text. This row tracks the requested presentation, not the existing mana value. |
