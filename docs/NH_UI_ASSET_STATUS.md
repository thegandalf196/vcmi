...
| Blue-buffer spell-points UI | Provisional | Not done | [SpellPointPresentation.h](../client/windows/SpellPointPresentation.h) supplies total/maximum, blue Buffer annotation and explanatory tooltips. Hero, kingdom, exchange, spellbook and battle surfaces have source bindings; narrow battle and compact hero tooltips use a separate blue Buffer line. Compact tooltip bindings await compilation and rendered checks, including long-value clipping; hidden maximum Mana is not queried. Earlier string tests and UI object compilation pass, but full runtime integration, promotion and rendered acceptance remain pending. The requested sparkle artwork is not bound; a preview mockup is not a runtime asset. |
